...
def figure_of_wire_path(
    ...
    path = spiral(outer_diam3, inner_diam, wire_diam, segment_count)
    spiral_1 = (
        path + np.array((-element_distance / 2, 0, -winding_casing_distance))[:, None] # Global position of the  coil
    )

    path = spiral2(outer_diam4, inner_diam, wire_diam, segment_count)
    spiral_2 = np.fliplr(
...
tms_coil.write("BobinaL4_PGH_05A.tcd")

...
def spiral2_coil1(outer_diam2: float, inner_diam: float, wire_diam: float, segment_count: int):
    ...
    # Evenly spaced angles
    phi = np.linspace(0, 2*np.pi*N2, segment_count)
    # Calculates the radius of every line segment in the spiral
    radius_loop = outer_diam / 2 - wire_diam / 2 * phi / (2 * np.pi)
    #Coil pitch
...
def figure_of_8_wire_path(
    ...
    spiral_1 = (
        path + np.array((0, 6.551, -winding_casing_distance))[:, None] # Global position of the first coil
    )

    path = spiral2_coil1(outer_diam2, inner_diam, wire_diam, segment_count)
    spiral_2 = np.fliplr(
        path + np.array((0, 6.551, -winding_casing_distance))[:, None] # Global position of the first core
    )

    path = spiral1_coil2(outer_diam, inner_diam, wire_diam, segment_count2)
    spiral_3 = np.fliplr(
        path + np.array((6.904, 0, -winding_casing_distance))[:, None] # Global position of the second coil
    )

    path = spiral2_coil2(outer_diam2, inner_diam, wire_diam, segment_count)
    spiral_4 = np.fliplr(
        path + np.array((6.904, 0, -winding_casing_distance))[:, None] # Global position of the second core
    )

    path = spiral1_coil3(outer_diam, inner_diam, wire_diam, segment_count)
    spiral_5 = (
        path + np.array((0, -6.551, -winding_casing_distance))[:, None] # Global position of the third coil
    )

    path = spiral2_coil3(outer_diam2, inner_diam, wire_diam, segment_count)
    spiral_6 = np.fliplr(
        path + np.array((0, -6.551, -winding_casing_distance))[:, None] # Global position of the third core
    )

    path = spiral1_coil4(outer_diam, inner_diam, wire_diam, segment_count)
    spiral_7 = np.fliplr(
        path + np.array((-6.904, 0, -winding_casing_distance))[:, None] # Global position of the fourth coil
    )

    path = spiral2_coil4(outer_diam2, inner_diam, wire_diam, segment_count)
    spiral_8 = np.fliplr(
        path + np.array((-6.904, 0, -winding_casing_distance))[:, None] # Global position of the fourth core
    )
...
tms_coil.write("SisEstL5_PGH_05A.tcd")

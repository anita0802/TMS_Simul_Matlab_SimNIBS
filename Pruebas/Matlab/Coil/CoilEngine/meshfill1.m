function [P, t, nodesadded] = meshfill1(p, e, normal)
    ...
    for m = 1:N
        ...
        %------------------------------------------------------------------
        edges               = meshconnee(dt.ConnectivityList);
        temp                = P(edges(:, 1), :) - P(edges(:, 2), :);
        MeshAvgEdgeLength   = mean(sqrt(dot(temp, temp, 2)));
        if (MeshAvgEdgeLength<0.5*AvgEdgeLength)
            break
        end
        %------------------------------------------------------------------
    end

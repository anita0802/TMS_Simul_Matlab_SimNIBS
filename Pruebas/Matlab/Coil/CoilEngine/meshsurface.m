function [P, t, normals] = meshsurface(Pcenter, a, b, M, flag, twist)
%   Outputs a 2-manifold P, t mesh for a single arbitrarily bent cylinder
%   with an arbitrary cross-section. The cylinder could be either open or
%   closed (torus). In the last case, the start point and the end point
%   must coincide.
%   Inputs:
...
%   cross-section);
%   flag is equal to one for the elliptical cross-sections and equals two for
%   the rectangular cross-section
%   Outputs:
%   P  -  P-aray of surface mesh vertices
%   t  -  t-array of surface triangular facets
%   Note: to create the mesh for end caps, either meshfill1 or meshfill2
%   can be used
%
%   SNM 2018-2021
    ...
    for m = 1:Nodes - 1        
        ...
        if m >= 1 & m < Nodes - 1
            ...
            ptop            = meshrotate2(ptop, UnitPathVector, (m-1)*twist);
    ...
    if ~Closed  
        ...
        [p1, tcapstart, added]  = meshfill2(pbottom, e);         
        t1                      = tcapstart;

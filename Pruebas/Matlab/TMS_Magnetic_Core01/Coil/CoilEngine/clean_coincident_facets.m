function [P, t, normals, centroids, areas, Indicator, parin, parout, contrast] = clean_coincident_facets(P, t, normals, centroids, areas, Indicator, parin, parout, contrast)
...
%   Copyright WAW/SNM 2019-2020
    ...
    %---Update paructivity information for duplicated facets---
    disp('  Resolving duplicate facets ...');
    if(~isempty(coincidentFacets))
        keepFacet = coincidentFacets(:,1);
        deleteFacet = coincidentFacets(:,2);
        parout(keepFacet) = parin(deleteFacet);
        contrast(keepFacet) = (parin(keepFacet) - parout(keepFacet))./(parin(keepFacet) + parout(keepFacet));
        ...
        parin(coincidentFacets(:,2),:) = [];
        parout(coincidentFacets(:,2),:) = [];

...
def spiral_core(outer_diam_core: float, segment_count: int):
    ...
    # Evenly spaced angles
    phi = np.linspace(0, 2*np.pi*N3, segment_count)     # core
    # Coil pitch
...
def figure_of_wire_path(
    ...
    #path[1, :] -= path[1, 0]  # y_base = 0 para el primer punto
   ...
   # path[1, :] -= path[1, 0]  # y_base = 0 para el primer punto
    ...
    #path[1, :] -= path[1, 0]  # y_base = 0 para el primer punto
    spiral_3 = path + np.array((0, 0, -winding_casing_distance))[:, None]
